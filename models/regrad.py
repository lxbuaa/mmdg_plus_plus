...
# @torchsnooper.snoop()
def calculate_prototype(model, dataloader, epoch, color_proto=None, ir_proto=None,
    ...
    with torch.no_grad():
        ...
        for step, sample_batched in enumerate(dataloader):
            ...
            logits = forward_model_with_domain(
                model, inputs, inputs_depth, inputs_ir, 'RGBDIR', sample_batched['domain']
            )
            ...
            # sample_count += 1
            # if sample_count >= all_num * sample_scale:
            #     break
...
def backward_regrad_3_modal_no_leak(model, optimizer, loss_dict):
    ...
    optimizer.zero_grad()
    loss_total.backward(retain_graph=False)  # last loss
